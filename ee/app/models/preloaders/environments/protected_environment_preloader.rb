  ...
  module Environments
    class ProtectedEnvironmentPreloader
      ...
      def execute(association_attributes)
        ...
        # Every ancestor group can protect the same tier, so group-level rows are
        # grouped rather than indexed: keeping only one would drop a parent group's
        # stricter rule and let the subgroup's looser one decide access on its own.
        project_protected_environments = associated_protected_environments.select(&:project_level?).index_by(&:name)
        group_protected_environments = associated_protected_environments.select(&:group_level?).group_by(&:name)
