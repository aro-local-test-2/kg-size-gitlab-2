  ...
  class ApprovalSummary
    ...
    def rules
      strong_memoize(:rules) do
        ...
        # Rules can be shared by several deployments' summaries when their
        # environments share protected environment instances, so each summary
        # attaches its approvals to a shallow copy.
        associated_approval_rules.map do |rule|
          rule.clone.tap { |summary_rule| summary_rule.approvals_for_summary = approvals_by_rule_id[rule.id] }
        end
      end
    end
  end
