...
module EE
  module ProjectImportState
    ...
    prepended do
      ...
      state_machine :status, initial: :none do
        ...
        after_transition [:scheduled, :started] => [:finished, :failed] do |state, _|
          ...
          # srem? returns false when the id is absent, which state_machines treats as a halt.
          nil
        end
        ...
        before_transition started: :finished do |state, _|
          if state.mirror?
            state.reindex_on_finish = !state.ever_updated_successfully?
    ...
    def reindex_on_finish?
      reindex_on_finish != false
    end
    ...
    def geo_handle_after_import_finished
      [project, project.wiki_repository, project.design_management_repository].compact.each do |container|
