...
export const i18n = {
  ...
  rotationCreatedReminder: s__(
    'OnCallSchedules|To enable notifications for this schedule, you must also create an %{linkStart}escalation policy%{linkEnd}.',
  ),
};

export default {
  name: 'OncallSchedulesWrapper',
  i18n,
  addScheduleModalId,
  components: {
    GlAlert,
  ...
  data() {
    return {
      ...
      showEscalationPolicyReminder: false,
  ...
  methods: {
    // Creating a rotation replaces the schedule tip alert, so the escalation
    // policy reminder from that tip is repeated here to keep the step visible.
    onRotationUpdate(message, { created = false } = {}) {
      this.showScheduleCreatedNotification = false;
      this.showRotationUpdatedNotification = true;
      this.showEscalationPolicyReminder = created;
...
<template>
  <div>
    ...
    <template v-else-if="hasSchedules">
      ...
      >
        {{ rotationUpdateMsg }}
        <span v-if="showEscalationPolicyReminder" data-testid="escalation-policy-reminder">
          <gl-sprintf :message="$options.i18n.rotationCreatedReminder">
            <template #link="{ content }">
              <gl-link :href="escalationPoliciesPath" target="_blank">{{ content }}</gl-link>
            </template>
          </gl-sprintf>
    ...
    <add-schedule-modal
      ...
      @schedule-created="showScheduleCreatedNotification = true"
    />
  </div>
</template>
