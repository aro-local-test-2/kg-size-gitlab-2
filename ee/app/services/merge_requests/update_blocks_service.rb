...
module MergeRequests
  class UpdateBlocksService
    ...
    def update?
      Gitlab::Utils.to_boolean(params[:update], default: false)
    end

    def remove_hidden?
      Gitlab::Utils.to_boolean(params[:remove_hidden], default: false)
    end
    ...
    def delete_old_blockers!(valid_references)
      merge_request
        ...
        .each do |block|
          ::MergeRequests::DestroyBlockService.new(user: current_user, block: block).execute
        end
    end
    ...
    def create_blockers(valid_references)
      ...
      new_ids.each_with_object([]) do |blocking_id, errors|
        result = ::MergeRequests::CreateBlockService.new(
          user: current_user,
          merge_request: merge_request,
