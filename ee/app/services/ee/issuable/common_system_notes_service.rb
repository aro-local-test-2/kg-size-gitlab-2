  ...
  module Issuable
    module CommonSystemNotesService
      ...
      def handle_date_change_note
        # Only legacy Epics track dates on their own record. For Issues and WorkItems, `issues.start_date`
        # is a denormalized sorting column and the change is already covered by the CE
        # `start_date_or_due_date` note, so noting it here would duplicate that note.
        return unless issuable.is_a?(::Epic)
