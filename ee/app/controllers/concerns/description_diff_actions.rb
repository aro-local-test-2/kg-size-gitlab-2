...
module DescriptionDiffActions
  ...
  def delete_description_version
    description_version.delete!(start_id: description_diff_params[:start_version_id])
  ...
  def description_diff_params
    params.permit(:start_version_id, :version_id)
  end

  def previous_description_version
    strong_memoize(:previous_description_version) do
      start_version_id = description_diff_params[:start_version_id]

      if start_version_id.present?
        issuable.description_versions.visible.find(start_version_id).previous_version
      else
  ...
  def description_version
    strong_memoize(:description_version) do
      issuable.description_versions.visible.find(description_diff_params[:version_id])
    end
  end
