...
RSpec.describe API::Members, feature_category: :groups_and_projects do
  ...
  context 'group members endpoints for group with minimal access feature' do
    ...
    let_it_be(:subgroup) { create(:group, parent: group, owners: owner) }
    ...
    describe 'PUT /groups/:id/members/:user_id' do
      ...
      context 'when member_role_id param is present' do
        ...
        let_it_be_with_reload(:member) { create(:group_member, :guest, source: group) }
        ...
        context "when custom roles license is enabled" do
          ...
          context 'when on SaaS' do
            ...
            context 'when member_role is associated with root group of subgroup membership' do
              let(:subgroup) { create(:group, parent: group, owners: owner) }
            ...
            context 'when member_role_id is nil' do
              ...
              it 'unsets the member_role_id attribute for the member' do
                ...
                expect(response).to have_gitlab_http_status(:ok)
                expect(json_response['id']).to eq(member.user_id)
                expect(json_response['member_role']).to be_nil
  ...
  context 'group members endpoint for enterprise users', :saas do
    ...
    context 'group with LDAP group link' do
      ...
      let_it_be(:group) { create(:group_with_ldap_group_link, owners: owner) }
      let_it_be_with_reload(:ldap_user) { create(:user) }
  ...
  context 'billable member endpoints' do
    ...
    describe 'GET /groups/:id/billable_members', feature_category: :seat_cost_management do
      ...
      context 'with sub group and projects' do
        ...
        context 'when a user has memberships in multiple sources' do
          before do
            project.add_maintainer(nested_user)
          end
      ...
      context 'email' do
        let_it_be_with_reload(:member) { create(:group_member, :developer, group: group) }
      ...
      context 'when a user has accepted an email invitation and has a membership in another source' do
        let_it_be_with_reload(:invited_user) { create(:user, email: 'invited@example.com') }
        let_it_be(:other_project) { create(:project, group: group) }

        before do
          member = create(:group_member, :invited, :developer, group: nested_group, invite_email: invited_user.email)
          member.accept_invite!(invited_user)

          other_project.add_maintainer(invited_user)
        end

        it 'does not return duplicate users' do
          get_billable_members

          expect_paginated_array_response(*[owner, maintainer, nested_user, invited_user].map(&:id))
        end
      end
    end
    ...
    describe 'PUT /groups/:id/members/:user_id/state', :saas do
      ...
      context 'when authenticated as an owner' do
        ...
        it_behaves_like 'authorizing granular token permissions', :toggle_state_member do
          let_it_be(:member) { create(:group_member, :awaiting, group: group) }
          let(:user) { owner }
          let(:boundary_object) { group }
          let(:request) do
  ...
  context 'without LDAP' do
    ...
    describe 'POST /projects/:id/members' do
      context 'group membership locked' do
        let(:user) { create(:user) }
        let(:group) { create(:group, membership_lock: true, owners: owner) }
        let(:project) { create(:project, group: group) }

        context 'project in a group' do
          it 'returns a 405 error with an explanatory message when group membership lock is enabled' do
            post api("/projects/#{project.id}/members", owner),
              params: { user_id: user.id, access_level: Member::MAINTAINER }

            expect(response).to have_gitlab_http_status(:method_not_allowed)
            expect(json_response['message'])
              .to eq('Members cannot be added because the group has locked project membership')
          end
        end
      end
    end
        ...
        context 'and current_user is group owner' do
          ...
          context 'when a user has a scim identity' do
            ...
            it 'returns a list of users with group SCIM identities info' do
              ...
              expect(json_response.second['group_scim_identity']).to be_nil
            end
          end

          context 'subgroup' do
            let(:subgroup) { create(:group, parent: group, owners: owner, maintainers: maintainer) }
  ...
  context 'group with LDAP group link' do
    ...
    let_it_be(:developer) { create(:user) }
    let_it_be(:ldap_developer) { create(:user) }
    let_it_be(:ldap_developer2) { create(:user) }

    let_it_be(:group) { create(:group_with_ldap_group_link, :public, owners: owner) }

    let_it_be_with_reload(:ldap_member) { create(:group_member, :developer, group: group, user: ldap_developer, ldap: true) }
    let_it_be_with_reload(:overridden_member) { create(:group_member, :developer, group: group, user: ldap_developer2, ldap: true, override: true) }
    let_it_be_with_reload(:regular_member) { create(:group_member, :developer, group: group, user: developer, ldap: false) }
    ...
    describe 'GET /groups/:id/members/:user_id' do
      it 'does not contain an override attribute for non-LDAP users in the response' do
        ...
        expect(json_response['override']).to be_nil
      end
      ...
      it 'contains an override attribute for ldap users in the response' do
        ...
        expect(json_response['access_level']).to eq(Member::DEVELOPER)
        expect(json_response['override']).to be(false)
      end
    end
    ...
    describe 'PUT /groups/:id/members/:user_id' do
      it 'succeeds when access_level is modified after override has been set' do
        ...
        expect(response).to have_gitlab_http_status(:ok)
        expect(json_response['id']).to eq(ldap_developer.id)
        expect(json_response['override']).to be(true)
  ...
  context 'group with pending members' do
    let_it_be_with_reload(:owner) { create(:user, username: 'owner_user') }
    let_it_be(:group) { create(:group, :public, owners: owner) }
    let_it_be(:subgroup) { create(:group, parent: group) }
    let_it_be(:project) { create(:project, group: group) }
    let_it_be_with_reload(:developer) { create(:user) }
    let_it_be_with_reload(:not_an_owner) { create(:user) }
    ...
    describe 'GET /groups/:id/pending_members' do
      ...
      context 'when the current user is authorized' do
        ...
        let_it_be_with_reload(:pending_invited_member) { create(:group_member, :awaiting, :invited, group: group) }
        ...
        context 'when the pending member has private email visibility' do
          it 'returns nil email' do
            get api(url, owner)

            pending_member = json_response.find { |m| m['id'] == pending_group_member.id }

            expect(pending_member['email']).to be_nil
          end
        end

        context 'when pending member has public email visibility' do
          let_it_be_with_reload(:user_with_public_email) { create(:user, :public_email, email: 'public@example.com') }
          let_it_be(:pending_group_member) { create(:group_member, :awaiting, group: group, user: user_with_public_email) }

          it 'returns the actual email' do
            get api(url, owner)

            pending_member = json_response.find { |m| m['id'] == pending_group_member.id }

            expect(pending_member['email']).to eq('public@example.com')
          end
        end
  ...
  context 'with billable member management' do
    ...
    describe 'PUT /groups/:id/members/:user_id' do
      let(:source) { group }
      let_it_be_with_reload(:membership) { create(:group_member, :guest, group: group, user: user) }
    ...
    describe 'PUT /projects/:id/members/:user_id' do
      let(:source) { project }
      let_it_be_with_reload(:membership) { create(:project_member, :guest, project: project, user: user) }
  ...
  context 'with the seat assignment model', :saas do
    let_it_be(:owner) { create(:user) }
    let_it_be(:user) { create(:user) }
    let_it_be(:group) { create(:group, :seat_assignment_model_enabled, owners: owner) }
    let_it_be(:subgroup) { create(:group, parent: group) }
    let_it_be(:project) { create(:project, group: subgroup) }

    let!(:seat_assignment) do
      create(:gitlab_subscription_seat_assignment, namespace: group, user: user, seat_type: :free)
    end

    shared_examples 'seat type enforcement when adding a member' do
      context 'when the seat_assignment_model feature flag is disabled' do
        ...
        it 'ignores the seat type and adds the member' do
          expect { post_member }.to change { source.members.count }.by(1)

          expect(response).to have_gitlab_http_status(:created)
        end
      end

      context 'when the seat type permits the role' do
        let(:access_level) { Gitlab::Access::GUEST }
      ...
      context 'when the seat type does not permit the role' do
        ...
        it 'does not add the member and returns a bad request error' do
          expect { post_member }.not_to change { source.members.count }

          expect(response).to have_gitlab_http_status(:bad_request)
          expect(json_response['message']).to eq(
            { 'access_level' => ['is not permitted for the seat type'] }
          )
        end
      end

      context 'when the seat type permits the custom role' do
        let(:access_level) { Gitlab::Access::GUEST }
        let(:member_role) { create(:member_role, :non_billable, namespace: group) }
        let(:params) { { user_id: user.id, access_level: access_level, member_role_id: member_role.id } }

        before do
          stub_licensed_features(custom_roles: true)
        end

        it 'adds the member with the custom role' do
          expect { post_member }.to change { source.members.count }.by(1)

          expect(response).to have_gitlab_http_status(:created)
          expect(source.members.with_user(user).first.member_role_id).to eq(member_role.id)
        end
      end

      context 'when the seat type does not permit the custom role' do
        let(:access_level) { Gitlab::Access::GUEST }
      ...
      context 'when adding multiple members' do
        ...
        let(:access_level) { Gitlab::Access::DEVELOPER }
        let(:params) { { user_id: [permitted_user.id, rejected_user.id].join(','), access_level: access_level } }

        before do
          create(:gitlab_subscription_seat_assignment, namespace: group, user: permitted_user, seat_type: :base)
          create(:gitlab_subscription_seat_assignment, namespace: group, user: rejected_user, seat_type: :free)
        end

        it 'adds permitted members and returns an error for rejected members' do
          expect { post_member }.to change { source.members.count }.by(1)

          expect(response).to have_gitlab_http_status(:created)
          expect(json_response['status']).to eq('error')
          expect(json_response['message']).to eq(
            "#{rejected_user.username}: Access level is not permitted for the seat type"
          )
        end
      end
    end

    shared_examples 'seat type enforcement when updating a member' do
      context 'when the seat_assignment_model feature flag is disabled' do
        let(:access_level) { Gitlab::Access::DEVELOPER }
        ...
        it 'ignores the seat type and updates the member' do
          expect { put_member }.to change { member.reload.access_level }.to(Gitlab::Access::DEVELOPER)
      ...
      context 'when the seat type permits the role' do
        let(:access_level) { Gitlab::Access::DEVELOPER }
        let!(:seat_assignment) do
          create(:gitlab_subscription_seat_assignment, namespace: group, user: user, seat_type: :base)
        end
      ...
      context 'when the seat type does not permit the role' do
        ...
        it 'does not update the member and returns a bad request error' do
          expect { put_member }.not_to change { member.reload.access_level }
      ...
      context 'when the seat type permits the custom role' do
        let(:access_level) { Gitlab::Access::GUEST }
        let(:member_role) { create(:member_role, :non_billable, namespace: group) }
        let(:params) { { access_level: access_level, member_role_id: member_role.id } }

        before do
          stub_licensed_features(custom_roles: true)
        end

        it 'assigns the custom role' do
          expect { put_member }.to change { member.reload.member_role_id }.to(member_role.id)
      ...
      context 'when the seat type does not permit the custom role' do
        ...
        it 'does not assign the custom role and returns a bad request error' do
          expect { put_member }.not_to change { member.reload.member_role_id }
    ...
    describe 'POST /groups/:id/members' do
      let(:source) { group }
      let(:params) { { user_id: user.id, access_level: access_level } }

      subject(:post_member) do
        post api("/groups/#{source.id}/members", owner), params: params
      end

      it_behaves_like 'seat type enforcement when adding a member'
    end
    ...
    describe 'POST /projects/:id/members' do
      ...
      subject(:post_member) do
        post api("/projects/#{source.id}/members", owner), params: params
      end
    ...
    describe 'PUT /groups/:id/members/:user_id' do
      let_it_be_with_reload(:member) { create(:group_member, :guest, source: group, user: user) }
      let(:params) { { access_level: access_level } }

      subject(:put_member) do
        put api("/groups/#{group.id}/members/#{user.id}", owner), params: params
      end

      it_behaves_like 'seat type enforcement when updating a member'
    end

    describe 'PUT /projects/:id/members/:user_id' do
      let_it_be_with_reload(:member) { create(:project_member, :guest, source: project, user: user) }
      let(:params) { { access_level: access_level } }

      subject(:put_member) do
        put api("/projects/#{project.id}/members/#{user.id}", owner), params: params
      end
