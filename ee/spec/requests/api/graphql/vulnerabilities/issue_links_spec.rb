...
RSpec.describe 'Query.vulnerabilities.issueLinks', feature_category: :vulnerability_management do
  ...
  let_it_be(:user) { create(:user, developer_of: project, security_dashboard_projects: [project]) }
  ...
  describe 'loading issue links in batch' do
    ...
    it 'does not cause N+1 query issue' do
      ...
      # Was 40. The token's last-used IP is now recorded; CI measures 44.
      # See https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/work_items/29728
      expect { query_issue_links }.not_to exceed_query_limit(44)
    end
  end
