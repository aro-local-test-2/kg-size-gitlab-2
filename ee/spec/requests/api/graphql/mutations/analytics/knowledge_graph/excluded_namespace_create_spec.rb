...
RSpec.describe 'KnowledgeGraphExcludedNamespaceCreate mutation', :enable_admin_mode,
  ...
  let(:mutation) { graphql_mutation(:knowledge_graph_excluded_namespace_create, group_path: group.full_path) }
  ...
  it_behaves_like 'authorizing granular token permissions for GraphQL',
    ...
    let(:mutation) do
      graphql_mutation(:knowledge_graph_excluded_namespace_create, { group_path: group.full_path }, 'errors')
    end

    let(:request) { post_graphql_mutation(mutation, token: { personal_access_token: pat }) }
  end

  it 'excludes the group' do
    expect { post_graphql_mutation(mutation, current_user: current_user) }
      .to change { Analytics::KnowledgeGraph::ExcludedNamespace.count }.by(1)

    expect(graphql_mutation_response(:knowledge_graph_excluded_namespace_create)).to include(
  ...
  it 'handles a concurrent duplicate exclusion' do
    allow_next_instance_of(Analytics::KnowledgeGraph::ExcludedNamespace) do |exclusion|
