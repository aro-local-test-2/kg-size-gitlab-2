...
RSpec.describe 'Setting the escalation policy of an issue', feature_category: :team_planning do
  ...
  it 'sets given escalation_policy to the escalation status for the issue' do
    # Was the default 100. The request IP is now set before rate limiting
    # resolves the user, so the token's last-used IP is recorded; CI measures 102.
  ...
  it_behaves_like 'authorizing granular token permissions for GraphQL', :update_issue do
    ...
    let(:mutation) do
      graphql_mutation(
        :issue_set_escalation_policy,
        { project_path: project.full_path, iid: issue.iid.to_s, escalation_policy_id: global_id_of(escalation_policy) },
        'errors'
      )
    end
