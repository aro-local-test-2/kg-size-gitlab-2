...
RSpec.describe API::ProtectedEnvironments, feature_category: :continuous_delivery do
  ...
  shared_examples 'denies access when CI/CD is disabled' do
    let_it_be(:project) do
      create(:project, :repository, builds_access_level: ProjectFeature::DISABLED)
    end

    it { expect { request }.to be_denied_for(:maintainer).of(project) }
    it { expect { request }.to be_denied_for(:owner).of(project) }
  end
  ...
  shared_examples 'requests to update approval rules' do
    ...
    it 'updates the environment / deleting approval rule without additional attributes' do
      create(:protected_environment_approval_rule, protected_environment: protected_environment, user_id: user.id)
  ...
  describe 'GET /projects/:id/protected_environments' do
    ...
    context 'when authenticated as a maintainer' do
      ...
      it_behaves_like 'denies access when CI/CD is disabled'
    end
  ...
  describe 'POST /projects/:id/protected_environments/' do
    ...
    context 'when authenticated as a maintainer' do
      ...
      it_behaves_like 'denies access when CI/CD is disabled' do
        let(:request) do
          post api_url, params: { name: 'staging', deploy_access_levels: [{ access_level: Gitlab::Access::MAINTAINER }] }
        end
      end
  ...
  describe 'PUT /projects/:id/protected_environments/:name' do
    ...
    context 'when authenticated as maintainer' do
      ...
      it_behaves_like 'denies access when CI/CD is disabled' do
        let(:request) do
          put api_url, params: { deploy_access_levels: [{ access_level: Gitlab::Access::MAINTAINER }] }
        end
      end
    end
  ...
  describe 'GET /groups/:id/protected_environments/:environment' do
    ...
    context 'when authenticated as a owner' do
      ...
      context 'when protected environment does not exist' do
        # A valid tier that simply is not protected: still a miss, not a malformed request.
        let(:requested_environment_name) { 'staging' }
      ...
      context 'when the name is not an environment tier' do
        let(:requested_environment_name) { 'unknown' }
        ...
        it 'returns 400' do
          ...
          expect(json_response['error']).to include('name does not have a valid value')
        end
      end
    end
  end
