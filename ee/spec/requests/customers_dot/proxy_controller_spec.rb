...
RSpec.describe CustomersDot::ProxyController, feature_category: :customersdot_and_quote_to_cash_integrations do
  describe 'POST graphql' do
    ...
    context 'with user signed in', :saas_cdot_graphql_proxy do
    ...
    context 'with no user signed in', :saas_cdot_graphql_proxy do
      let(:headers) { default_headers }

      it_behaves_like 'customersdot proxy'
    end

    context 'when the cdot_graphql_proxy SaaS feature is unavailable' do
      before do
        stub_saas_features(cdot_graphql_proxy: false)
      end

      it 'responds with 404 and does not forward the request to customers dot' do
        stub_request(:post, customers_dot)

        post customers_dot_proxy_graphql_path

        expect(response).to have_gitlab_http_status(:not_found)
        expect(WebMock).not_to have_requested(:post, customers_dot)
      end
