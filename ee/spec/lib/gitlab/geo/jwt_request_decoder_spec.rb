...
RSpec.describe Gitlab::Geo::JwtRequestDecoder, feature_category: :geo_replication do
  ...
  describe '#valid_scope?' do
    let(:data) { { scope: 'group/project' } }

    it 'returns true when the decoded scope matches' do
      expect(subject.valid_scope?('group/project')).to be_truthy
    end

    it 'returns false when the decoded scope differs' do
      expect(subject.valid_scope?('group/other')).to be_falsey
    end

    it 'returns false when the header cannot be decoded' do
      expect(described_class.new('invalid').valid_scope?('group/project')).to be_falsey
    end
  end
end
