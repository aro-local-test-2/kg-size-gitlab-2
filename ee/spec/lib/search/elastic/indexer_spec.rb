...
RSpec.describe Search::Elastic::Indexer, feature_category: :global_search do
  include StubENV
  ...
  context 'with an indexed project', :elastic do
    ...
    shared_examples 'index up to the specified commit' do
      ...
      it 'leaves the index status untouched when the indexing fails' do
        ...
        expect { indexer.run }.to raise_error(Search::Elastic::Indexer::Error)
    ...
    context 'when indexing a HEAD commit', :elastic do
      ...
      it 'runs the indexing command with the right flags' do
        ...
        expect_popen.with(
          [
            ...
            "--commit-traversal-ids-enabled=true",
  ...
  context "when indexing a group's wiki", :elastic do
    # `freeze: false` is required here: GroupWiki is a PORO that memoizes
    # its repository; `deep_freeze` raises FrozenError. Keep the opt-out
    # (see gitlab-org/gitlab#602925).
    let_it_be(:wiki, freeze: false) { create(:group_wiki) }
  ...
  context 'when no aws credentials available' do
    ...
    before do
      allow(Search::Elastic::Client).to receive(:resolve_aws_credentials).and_return(nil)
    end
  ...
  context 'when aws credentials are available' do
    ...
    context 'when AWS config is enabled' do
      ...
      it 'credentials env vars will be included' do
        expect(Search::Elastic::Client).to receive(:resolve_aws_credentials).and_return(credentials)
      ...
      context 'when static credentials are set' do
        ...
        it 'uses static credentials to set env vars' do
          expect(Search::Elastic::Client).to receive(:resolve_aws_credentials).and_call_original
  ...
  def elasticsearch_config
    Gitlab::CurrentSettings.elasticsearch_config.merge(
      ...
      config[:url] = config[:url].map { |u| ::Search::Elastic::Helper.url_string(u) }
    end
  end
