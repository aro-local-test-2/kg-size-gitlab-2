...
RSpec.describe "Test case shortcuts", :js, feature_category: :quality_management do
  ...
  describe 'pressing "l"' do
    it "opens labels dropdown for editing" do
      # The shortcuts bundle is lazy-loaded, so the `l` binding can register after the page settles.
      wait_for('labels dropdown to open') do
        find('body').native.send_key('l')
        has_css?('.js-labels-block [data-testid="listbox-search-input"]', wait: 1)
      end
