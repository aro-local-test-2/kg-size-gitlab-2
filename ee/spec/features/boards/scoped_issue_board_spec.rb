...
RSpec.describe 'Scoped issue boards', :js, feature_category: :planning_views do
  ...
  let_it_be(:assigned_issue, freeze: false) { create(:issue, project: project, assignees: [user]) }
    ...
    context 'new board' do
      ...
      context 'weight' do
        ...
        it 'creates board filtering by weight' do
          ...
          expect(page).to have_selector('.board-card-title', text: issue_weight_1.title, count: 1)
  ...
  def update_board_scope(filter, value)
    ...
    # The board loads state including the name asynchronously
    # so we wait for that before proceeding
    expect(page).to have_no_field('board-new-name', with: '')
