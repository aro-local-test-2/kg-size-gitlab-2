...
RSpec.describe ApplicationHelper, feature_category: :geo_replication do
  ...
  describe '#page_class', feature_category: :duo_chat do
    let(:renders) { true }
    let(:component) { instance_double(::DuoChatPanel::Component, render?: renders) }

    subject(:page_class) { helper.page_class.flatten }

    before do
      allow(::DuoChatPanel::Component).to receive(:new).and_return(component)
    end

    describe 'with-bloom' do
      context 'when the Duo chat panel renders' do
        it { is_expected.to include('with-bloom') }
      end

      context 'when the Duo chat panel does not render' do
        let(:renders) { false }

        it { is_expected.not_to include('with-bloom') }
      end

      it 'memoizes the panel render check across calls' do
        expect(component).to receive(:render?).once

        2.times { helper.page_class }
      end
    end
