  ...
  context 'when insights configuration project can be read by current user' do
    ...
    context 'when the configuration is attached to the current group' do
      ...
      describe 'POST #query.json' do
        ...
        it 'forwards only the keys the loader and validator read' do
          captured = nil

          allow(::Gitlab::Insights::Loader).to receive(:new).and_wrap_original do |original, **kwargs|
          ...
          expect(captured.keys).to match_array(%w[type query])
        end
      end
