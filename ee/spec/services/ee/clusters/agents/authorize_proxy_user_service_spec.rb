...
RSpec.describe Clusters::Agents::AuthorizeProxyUserService, feature_category: :deployment_management do
  ...
  context 'when user is an instance administrator without membership' do
    ...
    context 'when admin mode is enabled', :enable_admin_mode do
      it 'returns the authorizations with the roles of an owner', :aggregate_failures do
        owner_roles = %i[guest planner reporter security_manager developer maintainer owner]

        expect(service_response).to be_success
        expect(service_response.payload[:access_as]).to eq({
          user: {
            projects: [{ id: deployment_project.id, roles: owner_roles }],
            groups: [{ id: deployment_group.id, roles: owner_roles }]
          }
        })
      end
    end
