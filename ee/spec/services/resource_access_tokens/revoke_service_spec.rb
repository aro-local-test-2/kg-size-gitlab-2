...
RSpec.describe ResourceAccessTokens::RevokeService, feature_category: :system_access do
  subject(:revoke_access_token) { described_class.new(user, resource, access_token).execute }
  ...
  shared_examples 'audit event details' do
    it 'creates an audit event' do
      expect { revoke_access_token }.to change { AuditEventReader.count }.from(0).to(1)
    end

    it 'logs author and resource info', :aggregate_failures do
      revoke_access_token

      audit_event = AuditEventReader.where(author_id: user.id).last
