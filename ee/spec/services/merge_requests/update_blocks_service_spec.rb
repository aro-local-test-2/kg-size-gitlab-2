    ...
    context 'licensed' do
      ...
      context 'with string params' do
        context "with update: 'false'" do
          ...
          it 'keeps every blocking merge request' do
            service.execute

            expect(merge_request.blocking_merge_requests)
              .to contain_exactly(mr_to_keep, mr_to_del, hidden_mr)
          end
        end

        context "with update: 'true' and remove_hidden: 'false'" do
          let(:update) { 'true' }
          let(:remove_hidden) { 'false' }

          it 'keeps the hidden blocking merge request' do
            service.execute
        ...
        context "with update: 'true' and remove_hidden: 'true'" do
          let(:update) { 'true' }
          let(:remove_hidden) { 'true' }

          it 'removes the hidden blocking merge request' do
            service.execute
      ...
      context 'with remove_hidden: nil' do
        let(:update) { true }
        let(:remove_hidden) { nil }
