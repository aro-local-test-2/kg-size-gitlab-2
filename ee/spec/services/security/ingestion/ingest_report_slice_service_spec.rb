...
RSpec.describe Security::Ingestion::IngestReportSliceService, feature_category: :vulnerability_management do
  ...
  describe '#execute' do
    ...
    it 'runs IngestVulnerabilityNamespaceStatistics after the rest of the slice' do
      ingest_report_slice

      expect(Security::Ingestion::Tasks::HooksExecution).to have_received(:execute).ordered.with(pipeline, finding_maps)
      expect(Security::Ingestion::Tasks::IngestVulnerabilityNamespaceStatistics).to have_received(:execute).ordered.with(pipeline, finding_maps)
    end
    ...
    context 'when the feature flag is disabled' do
      ...
      it 'keeps IngestVulnerabilityNamespaceStatistics in the slice task list' do
        ingest_report_slice

        expect(Security::Ingestion::Tasks::IngestVulnerabilityNamespaceStatistics)
          .to have_received(:execute).with(pipeline, finding_maps)
      end
    end
