...
RSpec.describe SecretsManagement::UserPermissions::ProjectEffectiveCapabilitiesService,
  ...
  describe '#execute' do
    ...
    context 'with http_timeout' do
      subject(:service) do
        described_class.new(
          secrets_manager: secrets_manager, current_user: current_user, resource: project, http_timeout: 0.5
        )
      end
    ...
    it 'scopes the cache entry to the project', :use_clean_rails_memory_store_caching do
      stub_capabilities_self(data_caps: [], detailed_metadata_caps: %w[list])
