...
RSpec.describe ApprovalRules::UpdateService, feature_category: :code_review_workflow do
  ...
  shared_examples 'editable' do
    ...
    context 'when existing groups are inaccessible to user' do
      let_it_be(:private_accessible_group) { create(:group, :private, guests: user) }
      let_it_be(:private_inaccessible_group) { create(:group, :private) }
      let_it_be(:new_group, freeze: false) { create(:group) }
    ...
    context 'when user does not have right to edit' do
      ...
      it 'checks the update_approval_rule ability against the rule' do
        allow(Ability).to receive(:allowed?).and_call_original
        expect(Ability).to receive(:allowed?)
          .with(user, :update_approval_rule, approval_rule)
          .and_call_original
  ...
  context 'when target is project' do
    ...
    context 'when protected_branch_ids param is present' do
      ...
      context 'and multiple approval rules is enabled' do
        ...
        it 'associates the approval rule to the protected branch' do
          expect(result[:status]).to eq(:success)
          expect(result[:rule].protected_branches).to eq([protected_branch])
        end
        ...
        context 'but user cannot administer project' do
          ...
          it 'does not associate the approval rule to the protected branch' do
            expect(result[:status]).to eq(:success)
            expect(result[:rule].protected_branches).to be_empty
          end
        end

        context 'but protected branch is for another project' do
          let_it_be(:another_project, freeze: false) { create(:project) }
          let_it_be(:protected_branch, freeze: false) { create(:protected_branch, project: another_project) }
    ...
    describe 'audit events' do
      ...
      let_it_be(:approver, freeze: false) { create(:user, name: 'Batman') }
      let_it_be(:new_approver, freeze: false) { create(:user, name: 'Spiderman') }
      let_it_be(:parent_group) { create(:group, owners: user) }
      let_it_be(:destination) { create(:audit_events_group_external_streaming_destination, group: parent_group) }
      let_it_be(:project, freeze: false) do
        create(:project, creator: user, group: parent_group, reporters: [approver, new_approver])
      end

      let_it_be(:group, freeze: false) { create(:group, name: 'Justice League') }
      let_it_be(:new_group, freeze: false) { create(:group, name: 'Avengers') }
      ...
      context 'when licensed' do
        ...
        context 'when rule update operation succeeds', :request_store do
          ...
          it 'audits the group addition to approval group' do
            ...
            expect(AuditEventReader.last.details[:custom_message]).to eq(
  ...
  context 'when target is merge request' do
    let_it_be(:target) { create(:merge_request, source_project: project, target_project: project) }
  ...
  describe 'cross-organization group isolation' do
    ...
    let_it_be(:isolated_project, freeze: false) { create(:project, organization: organization) }
    let_it_be(:non_isolated_project, freeze: false) { create(:project, organization: other_organization) }
    let_it_be(:same_org_group) { create(:group, :private, organization: organization) }
    let_it_be(:cross_org_group) { create(:group, :private, organization: other_organization) }
    ...
    context 'when the organization is isolated' do
      let(:approval_rule) { isolated_project.approval_rules.create!(name: 'iso-rule', approvals_required: 1) }

      it 'silently filters out groups from a different organization' do
        result = described_class.new(approval_rule, actor, {
          group_ids: [same_org_group.id, cross_org_group.id]
        }).execute

        expect(result).to be_success
        expect(result[:rule].groups).to contain_exactly(same_org_group)
      end
    end

    context 'when the organization is not isolated' do
      let(:approval_rule) { non_isolated_project.approval_rules.create!(name: 'open-rule', approvals_required: 1) }

      it 'allows groups from any organization' do
        result = described_class.new(approval_rule, actor, {
          permit_inaccessible_groups: true,
          group_ids: [cross_org_group.id, same_org_group.id]
        }).execute

        expect(result).to be_success
        expect(result[:rule].groups).to contain_exactly(cross_org_group, same_org_group)
      end
    end
  end
  ...
  context 'when target is a group' do
    ...
    let_it_be(:container) { target }
    let_it_be(:group_user) { create(:user) }
    let_it_be(:user) { group_user }
