...
RSpec.describe Ai::DuoWorkflows::GenerateWorkplanService, feature_category: :duo_agent_platform do
  ...
  context 'when the workplan flow is unavailable' do
    before do
      allow(::Ai::Catalog::FoundationalFlow).to receive(:[]).with('workplan/v1').and_return(nil)
    end
  ...
  context 'when a workplan workflow is already running for the work item' do
  ...
  context 'when another request is already generating a workplan for the work item' do
    before do
      stub_exclusive_lease_taken("duo_workflows_generate_workplan:#{work_item.id}", timeout: 2.minutes.to_i)
    end
  ...
  context 'when a previous workplan workflow for the work item has already finished' do
  ...
  context 'when enabled and the workplan flow is available' do
    ...
    it 'starts the workplan flow with a goal built from the work item', :aggregate_failures do
      expect(::Ai::DuoWorkflows::CreateAndStartWorkflowService).to receive(:new) do |**kwargs|
        expect(kwargs[:container]).to eq(project)
        expect(kwargs[:resource]).to eq(work_item)
        expect(kwargs[:current_user]).to eq(current_user)
        expect(kwargs[:source_branch]).to eq(project.default_branch)
        expect(kwargs[:workflow_definition].foundational_flow_reference).to eq('workplan/v1')
        expect(kwargs[:goal]).to include(Gitlab::UrlBuilder.build(work_item))
        downstream
      end
