...
RSpec.describe License, feature_category: :plan_provisioning do
  ...
  describe 'validations' do
    ...
    describe '#check_trueup' do
      ...
      shared_examples 'invalid if active users with threshold exceeds restricted user count' do
        ...
        it 'is not valid' do
          expect(license).not_to be_valid
          expect(license.errors.added?(:base, :check_available_seats)).to be(true)
        end
      end
      ...
      context 'when reconciliation_completed is false on the license' do
        it 'adds errors for invalid true up figures' do
          ...
          expect(license).not_to be_valid
          expect(license.errors.added?(:base, :check_trueup)).to be(true)
          expect(license.errors.full_messages.to_sentence)
        ...
        with_them do
          ...
          context 'when license has unlimited users' do
            ...
            it 'is always valid' do
              expect(new_license.valid?).to be(true)
            end
          end
        end
      end
  ...
  describe 'Callbacks' do
    describe '.current', :request_store do
      it 'caches the result in SafeRequestStore' do
        described_class.current

        expect(Gitlab::SafeRequestStore.read(:current_license)).to be_present
      end

      it 'returns the same object instance within a request' do
  ...
  describe "Class methods" do
    ...
    describe '.current', :request_store, :use_clean_rails_memory_store_caching do
      ...
      context 'when the license is valid' do
        ...
        it 'caches the license' do
          ...
          expect(described_class).to receive(:load_license).at_most(:once).and_call_original
    ...
    describe ".block_changes?" do
      ...
      context 'with an expired normal license' do
        ...
        it 'returns true' do
          expect(described_class.block_changes?).to be(true)
        end
      end
    ...
    context 'with an expired trial license' do
      ...
      ::GitlabSubscriptions::Features::ALL_STARTER_FEATURES.each do |feature|
        it "returns false for #{feature}" do
          expect(described_class.feature_available?(feature)).to be(false)
        end
      end
    end
      ...
      context 'without license' do
          ...
          it 'does not have access to any usage ping features' do
            ::GitlabSubscriptions::Features::FEATURES_WITH_USAGE_PING.each do |feature|
              expect(described_class.feature_available?(feature))
                .to be(false), "expected #{feature} not to be available"
            end
          end
        end
        ...
        context 'when usage ping is enabled' do
          ...
          it 'has access to all usage ping features' do
            ::GitlabSubscriptions::Features::FEATURES_WITH_USAGE_PING.each do |feature|
              expect(described_class.feature_available?(feature))
                .to be(true), "expected #{feature} to be available"
            end
          end
        end
      end
        ...
        context 'when usage ping is disabled' do
          ...
          it 'has access to usage ping features part of the plan' do
            ::GitlabSubscriptions::Features::ALL_PREMIUM_FEATURES.each do |feature|
              expect(described_class.feature_available?(feature)).to be(true)
            end
          end
  ...
  describe '.has_active_gitlab_credits_add_on?' do
    subject { described_class.has_active_gitlab_credits_add_on? }

    context 'when instance has an active gitlab_credits add-on' do
      before do
    ...
    context 'when instance has no active gitlab_credits add-on' do
      it { is_expected.to be(false) }
    end
  end

  describe '.gitlab_credits_entitled?' do
    subject { described_class.gitlab_credits_entitled? }

    context 'when usage_billing licensed feature is available' do
      before do
  ...
  describe '#overage' do
    ...
    it 'returns the difference using daily_billable_users_count as user_count if no user_count argument provided' do
      allow(license).to receive_messages(daily_billable_users_count: 110, seats: 100)
  ...
  describe '.billable_license' do
    context 'when the current license is not a trial' do
      context 'when the current license is a paid plan' do
        ...
        it 'returns the current license' do
          expect(described_class.billable_license).to eq(license)
        end
      end

      context 'when the current license is not a paid plan but has an active credits add-on' do
        let(:free_license) { build_stubbed(:license) }

        before do
          allow(free_license).to receive(:paid?).and_return(false)
          allow(described_class).to receive_messages(current: free_license, has_active_gitlab_credits_add_on?: true)
        end

        it 'returns the current license' do
          expect(described_class.billable_license).to eq(free_license)
        end
      end

      context 'when the current license is not a paid plan and has no add-on' do
        let(:free_license) { build_stubbed(:license) }

        before do
          allow(free_license).to receive(:paid?).and_return(false)
          allow(described_class).to receive_messages(current: free_license, has_active_gitlab_credits_add_on?: false)
        end

        it 'returns nil' do
          expect(described_class.billable_license).to be_nil
        end
      end
    end

    context 'when the current license is a trial' do
      context 'when a prior active paid license exists' do
        ...
        it 'returns the prior paid license' do
          expect(described_class.billable_license).to eq(prior_license)
        end
      end

      context 'when a prior active paid license exists and a credits add-on is also active' do
        let!(:prior_license) { create(:license, plan: License::PREMIUM_PLAN) }

        before do
          create(:license, :ultimate_trial)
          described_class.reset_current
          allow(described_class).to receive(:has_active_gitlab_credits_add_on?).and_return(true)
        end

        it 'returns the prior paid license, not the current trial' do
          expect(described_class.billable_license).to eq(prior_license)
        end
      end

      context 'when the prior license is also a trial' do
        let!(:prior_trial) { create(:license, :ultimate_trial) }

        before do
          allow(described_class).to receive_messages(current: create(:license, :ultimate_trial), recent: described_class.where(id: prior_trial.id), has_active_gitlab_credits_add_on?: false)
        end
      ...
      context 'when the prior license is expired' do
        let!(:prior_license) { create(:license, plan: License::PREMIUM_PLAN, expired: true) }

        before do
          allow(described_class).to receive_messages(current: create(:license, :ultimate_trial), recent: described_class.where(id: prior_license.id), has_active_gitlab_credits_add_on?: false)
        end
      ...
      context 'when no prior paid license exists but a credits add-on is active' do
        before do
          allow(described_class).to receive_messages(current: create(:license, :ultimate_trial), recent: described_class.none, has_active_gitlab_credits_add_on?: true)
        end

        it 'returns the current license' do
          expect(described_class.billable_license).to eq(described_class.current)
        end
      end

      context 'when no prior licenses exist and no add-on' do
        before do
          allow(described_class).to receive_messages(current: create(:license, :ultimate_trial), recent: described_class.none, has_active_gitlab_credits_add_on?: false)
        end
    ...
    context 'when there is no current license' do
      before do
        allow(described_class).to receive_messages(current: nil, has_active_gitlab_credits_add_on?: false)
      end
    ...
    describe 'memoization', :request_store do
      ...
      it 'caches the result in SafeRequestStore' do
        described_class.billable_license

        expect(Gitlab::SafeRequestStore.read(:billable_license)).to be_present
      end

      it 'returns the same object instance within a request' do
        first_call = described_class.billable_license
        second_call = described_class.billable_license

        expect(first_call).to be(second_call)
      end

      it 'is cleared by reset_current' do
        described_class.billable_license
        expect(Gitlab::SafeRequestStore.read(:billable_license)).to be_present

        described_class.reset_current

        expect(Gitlab::SafeRequestStore.read(:billable_license)).to be_nil
      end

      it 'is cleared when a license is created' do
        described_class.billable_license
        expect(Gitlab::SafeRequestStore.read(:billable_license)).to be_present
  ...
  describe '#active_user_count_threshold_reached?' do
    ...
    with_them do
      before do
        allow(license).to receive_messages(daily_billable_users_count: daily_billable_users_count, seats: seats)
      end
  ...
  describe '#update_billable_user_counts' do
    subject(:create_license) { create(:license, data: create(:gitlab_license, starts_at: start_date).export) }

    before do
      allow(::Analytics::UsageTrends::CounterJobWorker).to receive(:perform_async)
    end

    context 'when the license has already started' do
      let(:start_date) { Date.current }

      it 'enqueues a CounterJobWorker job to refresh billable user counts' do
        create_license
    ...
    context 'when the license is future-dated' do
      ...
      it 'does not enqueue a CounterJobWorker job' do
        create_license
