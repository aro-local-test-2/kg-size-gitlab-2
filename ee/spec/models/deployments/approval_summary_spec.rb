...
RSpec.describe Deployments::ApprovalSummary do
  ...
  describe '#total_pending_approval_count' do
    ...
    context 'when another deployment shares the environment instance and its rules' do
      include_context 'with one rule has been approved'

      let(:other_deployment) do
        create(:deployment, project: project, environment: environment).tap do |other|
          other.environment = deployment.environment
        end
      end

      it 'does not read the other deployment approvals' do
        other_summary = described_class.new(deployment: other_deployment)

        expect(approval_summary.total_pending_approval_count).to eq(3)
        expect(other_summary.total_pending_approval_count).to eq(4)
        expect(approval_summary.total_pending_approval_count).to eq(3)
      end
    end
  end
