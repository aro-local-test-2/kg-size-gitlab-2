...
RSpec.describe ApprovalState, feature_category: :code_review_workflow do
  ...
  let_it_be_with_reload(:approver3) { create(:user) }
  let_it_be_with_reload(:default_approver) { create(:user) }
  let_it_be(:arbitrary_users, freeze: false) { create_list(:user, 2) }

  let_it_be_with_reload(:group_approver1) { create(:user) }
  let_it_be(:group1) do
    group = create(:group)
  ...
  shared_context 'project members' do
    ...
    let_it_be_with_refind(:project) { create(:project, :repository) }
    let_it_be(:author, freeze: false) { create_project_member(:developer) }
    let_it_be_with_refind(:merge_request) { create(:merge_request, source_project: project, author: author) }
    let_it_be(:approver, freeze: false) { create_project_member(:developer) }
    let_it_be(:approver2, freeze: false) { create_project_member(:developer) }
    let_it_be(:developer, freeze: false) { create_project_member(:developer) }
    let_it_be(:other_developer, freeze: false) { create_project_member(:developer) }
    let_it_be(:reporter, freeze: false) { create_project_member(:reporter) }
    let_it_be_with_reload(:stranger) { create(:user) }
  end

  describe '#approval_rules_overwritten?' do
    context 'when approval rule on the merge request does not exist' do
      it 'returns false' do
        expect(subject.approval_rules_overwritten?).to be(false)
      end
    end

    context 'when merge request has any approver rule' do
      let_it_be(:any_approver_rule) { create(:any_approver_rule, merge_request: merge_request) }

      it 'returns true' do
        expect(subject.approval_rules_overwritten?).to be(true)
      end
  ...
  describe '#overall_approver_ids' do
    let_it_be(:approval) { create(:approval, merge_request: merge_request, user: approver1) }

    before do
      stub_licensed_features(multiple_approval_rules: true)
    end

    it 'issues one approvals user ID query when multiple wrapped rules resolve their approval state', :aggregate_failures do
      3.times do
        create(:approval_merge_request_rule, merge_request: merge_request, users: [approver1], approvals_required: 1)
      end
      merge_request.approvals.reset

      recorder = ActiveRecord::QueryRecorder.new(skip_cached: false) do
        subject.wrapped_approval_rules.map(&:approved?)
      end
      approvals_user_id_queries = recorder.log.grep(/\ASELECT DISTINCT "approvals"\."user_id" FROM "approvals"/)

      expect(subject.wrapped_approval_rules.map(&:approved?)).to all(be(true))
      expect(approvals_user_id_queries.size).to eq(1)
    end

    it 'does not query approvals when approvals are already loaded', :aggregate_failures do
      merge_request.approvals.load

      recorder = ActiveRecord::QueryRecorder.new(skip_cached: false) do
        subject.overall_approver_ids
      end
      approvals_user_id_queries = recorder.log.grep(/\ASELECT DISTINCT "approvals"\."user_id" FROM "approvals"/)

      expect(subject.overall_approver_ids).to eq(Set[approver1.id])
      expect(approvals_user_id_queries).to be_empty
    end
  end
  ...
  context 'when multiple rules are allowed' do
    ...
    describe '#wrapped_approval_rules' do
      ...
      it 'does not cause N+1 queries when accessing approvers on wrapped rules' do
        create(:approval_merge_request_rule, merge_request: merge_request, users: [approver1], groups: [group1])

        control = ActiveRecord::QueryRecorder.new do
          fresh_mr = MergeRequest.find(merge_request.id)
          fresh_mr.approval_state.wrapped_approval_rules.each(&:approvers)
        end

        2.times do
          create(:approval_merge_request_rule, merge_request: merge_request, users: [create(:user)])
        end
    ...
    describe '#approval_needed?' do
      context 'when feature not available' do
        it 'returns false' do
          ...
          expect(subject.approval_needed?).to be(false)
        end
      end
      ...
      context 'when overall approvals required is not zero' do
        ...
        it 'returns true' do
          expect(subject.approval_needed?).to be(true)
        end
      end
    ...
    describe '#all_approval_rules_approved?' do
      context 'when all rules are approved' do
        ...
        it 'returns true' do
          expect(subject.all_approval_rules_approved?).to be(true)
        end
      ...
      context 'when some rules are not approved' do
        ...
        it 'returns false' do
          expect(subject.all_approval_rules_approved?).to be(false)
        end
    ...
    describe '#expire_unapproved_key!', :clean_gitlab_redis_shared_state do
      let(:coordinator) { ::MergeRequests::ApprovalRulesSyncCoordinator.new(merge_request) }

      it 'removes the legacy temporarily_unapprove flag' do
        subject.temporarily_unapprove!

        subject.expire_unapproved_key!

        expect(subject.temporarily_unapproved?).to be(false)
      end

      it 'clears in-progress sync state via the coordinator' do
      ...
      context 'when deterministic_approval_rules_sync is disabled' do
        before do
          stub_feature_flags(deterministic_approval_rules_sync: false)
        end
        ...
        it 'does not delegate to the coordinator' do
          expect(::MergeRequests::ApprovalRulesSyncCoordinator).not_to receive(:new)
    ...
    describe '#temporarily_unapprove!', :clean_gitlab_redis_shared_state do
      def key_ttl
        Gitlab::Redis::SharedState.with { |redis| redis.ttl("mr_#{merge_request.id}_cannot_merge") }
      end

      it 'defaults to the 5-minute TTL used by the create/code-owner-sync flow' do
        subject.temporarily_unapprove!

        expect(key_ttl).to be_within(1).of(ApprovalState::TEMPORARILY_UNAPPROVED_TTL.to_i)
      end

      it 'uses the supplied TTL when given (e.g. the short push/reset TTL)' do
        subject.temporarily_unapprove!(ApprovalState::TEMPORARILY_UNAPPROVED_TTL_ON_RESET)

        expect(key_ttl).to be_within(1).of(ApprovalState::TEMPORARILY_UNAPPROVED_TTL_ON_RESET.to_i)
      end

      it 'causes #temporarily_unapproved? to return true' do
        subject.temporarily_unapprove!

        expect(subject.temporarily_unapproved?).to be(true)
      end
    end
    ...
    describe '#temporarily_unapproved?', :clean_gitlab_redis_shared_state do
      ...
      it 'returns false when there are no flags or syncs' do
        expect(subject.temporarily_unapproved?).to be(false)
      end

      it 'returns true when the legacy flag is set' do
        subject.temporarily_unapprove!

        expect(subject.temporarily_unapproved?).to be(true)
      end

      it 'returns true when a sync is in progress' do
        coordinator.start_approval_rules_sync!(:code_owner, 'abc123')
      ...
      context 'when deterministic_approval_rules_sync is disabled' do
        ...
        it 'ignores syncing approval rules' do
          coordinator.start_approval_rules_sync!(:code_owner, 'abc123')
    ...
    describe '#eligible_for_approval_by?' do
      ...
      include_context 'project members' do
        let_it_be(:committer, freeze: false) { create_project_member(:developer, email: merge_request.commits.without_merge_commits.first.committer_email) }
      end
      ...
      context 'when there is one approver required' do
        let_it_be(:any_approver_rule) { create_rule(rule_type: :any_approver, approvals_required: 1) }
        let_it_be(:rule) { create_rule(approvals_required: 1, users: [author]) }
      ...
      context 'when any_approver rule with 2 approvals required exist' do
        ...
        context 'a user approves the MR' do
          ...
          it 'requires 1 approval' do
            expect(merge_request.approved_by?(approver)).to be(true)
            expect(subject.approvals_left).to eq(1)
          end
          ...
          context 'another user approves the MR' do
            ...
            it 'becomes approved' do
              expect(merge_request.approved_by?(approver1)).to be(true)
              expect(subject.approved?).to be(true)
            end
          end
        end
  ...
  context 'when only a single rule is allowed' do
    ...
    describe '#wrapped_approval_rules' do
      it 'returns one regular rule in wrapper' do
        ...
        subject.wrapped_approval_rules.each do |rule|
          expect(rule.is_a?(ApprovalWrappedRule)).to be(true)
        end
    ...
    describe '#eligible_for_approval_by?' do
      ...
      include_context 'project members' do
        let_it_be(:guest, freeze: false) { create_project_member(:guest) }
      end
      ...
      context 'when user is a committer' do
        let_it_be_with_reload(:user) { create(:user, email: merge_request.commits.without_merge_commits.first.committer_email) }
  ...
  describe '#any_approver_candidates' do
    include_context 'project members'

    let_it_be_with_reload(:maintainer) { create_project_member(:maintainer) }
    let_it_be_with_reload(:other_maintainer) { create_project_member(:maintainer) }
    ...
    let_it_be(:service_user) { create(:user, :service_user, developer_of: project) }

    def protect_target_branch(merge: [], push: [])
      create(:protected_branch,
        project: project,
        name: merge_request.target_branch,
        default_access_level: false
      ).tap do |protected_branch|
        merge.each { |level| protected_branch.merge_access_levels.create!(access_level: level) }
        push.each { |level| protected_branch.push_access_levels.create!(access_level: level) }
      end
    end

    context 'when the target branch is not protected' do
      it 'returns the active humans that can approve, excluding the author', :aggregate_failures do
        candidates = subject.any_approver_candidates(limit: 10)

        expect(candidates).to include(developer, maintainer)
        expect(candidates).not_to include(
          author, reporter, stranger, blocked_developer, project_bot, service_user
        )
      end

      it 'orders by recent activity and respects the limit' do
        developer.update!(last_activity_on: Date.current)

        expect(subject.any_approver_candidates(limit: 1)).to contain_exactly(developer)
      end
    end

    context 'when only maintainers can merge into the target branch' do
      before do
        protect_target_branch(merge: [Gitlab::Access::MAINTAINER])
      end

      it 'narrows the candidates to the members that can merge', :aggregate_failures do
        candidates = subject.any_approver_candidates(limit: 10)

        expect(candidates).to include(maintainer)
        expect(candidates).not_to include(developer, other_developer)
      end

      it 'orders by recent activity and respects the limit' do
        other_maintainer.update!(last_activity_on: Date.current)

        expect(subject.any_approver_candidates(limit: 1)).to contain_exactly(other_maintainer)
      end
    end

    context 'when developers can push to the target branch' do
      before do
        protect_target_branch(merge: [Gitlab::Access::MAINTAINER], push: [Gitlab::Access::DEVELOPER])
      end

      it 'includes developers, because push access allows merging' do
        expect(subject.any_approver_candidates(limit: 10)).to include(developer, maintainer)
      end
    end

    context 'when no one can merge into the target branch' do
      before do
        protect_target_branch(merge: [Gitlab::Access::NO_ACCESS], push: [Gitlab::Access::NO_ACCESS])
      end

      it 'falls back to every member that can approve' do
        expect(subject.any_approver_candidates(limit: 10)).to include(developer, maintainer)
      end
    end

    context 'when every member that can merge is inactive' do
      before do
        protect_target_branch(merge: [Gitlab::Access::MAINTAINER])

        [project.first_owner, maintainer, other_maintainer].each(&:block!)
      end

      it 'falls back to every member that can approve', :aggregate_failures do
        candidates = subject.any_approver_candidates(limit: 10)

        expect(candidates).to include(developer, other_developer)
        expect(candidates).not_to include(maintainer, other_maintainer)
      end
    end

    context 'when merge access is granted to a user rather than a role' do
      before do
        stub_licensed_features(protected_refs_for_users: true)

        protect_target_branch.merge_access_levels.create!(user: maintainer)
      end
    ...
    context 'when committers cannot approve' do
      before do
        project.update!(merge_requests_disable_committers_approval: true)

        allow(merge_request).to receive(:committer_ids_to_filter_from_approvers)
          .and_return(User.id_in(maintainer.id).select(:id))
      end

      it 'excludes the committers, who cannot satisfy the rule', :aggregate_failures do
        candidates = subject.any_approver_candidates(limit: 10)

        expect(candidates).to include(developer)
        expect(candidates).not_to include(maintainer)
      end
    end

    context 'when a member is banned from the namespace' do
      let_it_be(:banned_group) { create(:group) }
      let_it_be_with_refind(:banned_project) { create(:project, :repository, group: banned_group) }
      let_it_be(:banned_merge_request) { create(:merge_request, source_project: banned_project) }
      let_it_be(:banned_developer) { create(:user, developer_of: banned_project) }
      let_it_be(:other_developer) { create(:user, developer_of: banned_project) }

      let(:candidates) { banned_merge_request.approval_state.any_approver_candidates(limit: 10) }

      before_all do
        create(:namespace_ban, namespace: banned_group, user: banned_developer)
      end

      context 'when the download limit that issues the bans is enabled' do
        before do
          stub_licensed_features(unique_project_download_limit: true)
        end

        it 'excludes them, because the policy prevents them from approving' do
          expect(candidates).to contain_exactly(other_developer)
        end
      end

      it 'keeps them when the limit is disabled, because the ban does not prevent approval' do
        expect(candidates).to contain_exactly(banned_developer, other_developer)
      end
    end

    context 'when no direct member can approve' do
      let_it_be_with_refind(:memberless_project) { create(:project, :repository, group: create(:group)) }
      let_it_be(:memberless_merge_request) { create(:merge_request, source_project: memberless_project) }

      before_all do
        memberless_project.group.add_owner(create(:user))
      end

      it 'recommends nobody, rather than falling back to the inherited members' do
        candidates = memberless_merge_request.approval_state.any_approver_candidates(limit: 10)
    ...
    context 'when the project belongs to a group' do
      ...
      let_it_be_with_refind(:group_project) { create(:project, :repository, group: group) }
      let_it_be(:group_merge_request) { create(:merge_request, source_project: group_project) }
      let_it_be(:direct_maintainer) { create(:user, maintainer_of: group_project) }
      let_it_be(:inherited_owner) { create(:user, owner_of: group) }

      it 'excludes the members that only inherit their access from an ancestor group' do
        candidates = group_merge_request.approval_state.any_approver_candidates(limit: 10)

        expect(candidates).to contain_exactly(direct_maintainer)
      end

      context 'when a direct member has a higher access level through the group' do
        let_it_be(:direct_reporter) do
          create(:user, reporter_of: group_project).tap { |user| group.add_owner(user) }
        end

        it 'takes the access level from the inherited membership' do
          create(:protected_branch, :maintainers_can_merge, :no_one_can_push,
            project: group_project, name: group_merge_request.target_branch)

          candidates = group_merge_request.approval_state.any_approver_candidates(limit: 10)

          expect(candidates).to contain_exactly(direct_maintainer, direct_reporter)
        end
      end
    end
  end

  describe 'code owner rule grouping' do
    def create_code_owner(name:, section:, users: [approver1], groups: [])
    ...
    context 'when a section has multiple patterns sharing an approver set' do
      let_it_be(:frontend_group) { create(:group, developers: [group_approver1]) }

      before do
        create_code_owner(name: '*.vue', section: 'Frontend', users: [approver1, group_approver1])
        create_code_owner(name: '*.js', section: 'Frontend', users: [approver1], groups: [frontend_group])
      end

      it 'leaves wrapped_approval_rules ungrouped' do
        code_owner_rules = subject.wrapped_approval_rules.select(&:code_owner?)

        expect(code_owner_rules.map(&:name)).to contain_exactly('*.vue', '*.js')
        expect(code_owner_rules).to all(be_an_instance_of(ApprovalWrappedCodeOwnerRule))
      end

      it 'combines the patterns into a single grouped rule' do
        code_owner_rules = subject.grouped_approval_rules.select(&:code_owner?)

        expect(code_owner_rules.size).to eq(1)
        expect(code_owner_rules.first.patterns).to contain_exactly('*.vue', '*.js')
      end

      it 'counts the section once in the grouped counts' do
        expect(subject.grouped_approvals_required).to eq(1)
        expect(subject.grouped_approvals_left).to eq(1)
      end

      it 'counts every pattern in the ungrouped counts' do
        expect(subject.approvals_required).to eq(2)
        expect(subject.approvals_left).to eq(2)
      end
    end

    context 'when two sections share an approver set' do
      before do
        create_code_owner(name: '*.rb', section: 'Backend')
        create_code_owner(name: '*.sql', section: 'Database')
      end

      it 'keeps the sections separate' do
        expect(subject.grouped_approval_rules.select(&:code_owner?).map(&:patterns))
          .to contain_exactly(['*.rb'], ['*.sql'])
        expect(subject.grouped_approvals_required).to eq(2)
      end
    end

    context 'when a section has patterns with different approver sets' do
      before do
        create_code_owner(name: '*.rb', section: 'Maintainers', users: [approver1])
        create_code_owner(name: '*.rake', section: 'Maintainers', users: [approver1])
        create_code_owner(name: '/workhorse/', section: 'Maintainers', users: [approver2])
      end

      it 'groups and counts each approver set separately' do
        expect(subject.grouped_approval_rules.select(&:code_owner?).map(&:patterns))
          .to contain_exactly(contain_exactly('*.rb', '*.rake'), ['/workhorse/'])
        expect(subject.grouped_approvals_required).to eq(2)
      end
    end
  end
  ...
  describe 'v2_approval_rules' do
    ...
    it 'preloads approver_users and group_users to avoid N+1 queries when accessing approvers' do
      create(:merge_requests_approval_rule, :from_merge_request, :with_users, project_id: project.id, merge_request: merge_request, user: approver1)
      create(:merge_requests_approval_rule, :from_merge_request, :with_users, project_id: project.id, merge_request: merge_request, user: approver2)

      wrapped_rules = subject.wrapped_approval_rules

      expect do
        wrapped_rules.each do |wrapped_rule|
          wrapped_rule.approval_rule.approver_users.to_a
          wrapped_rule.approval_rule.group_users.to_a
        end
