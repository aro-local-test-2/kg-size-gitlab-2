...
describe('EE Approvlas MRRulesHiddenInputs', () => {
  ...
  describe('can edit', () => {
    ...
    describe('with rules', () => {
      ...
      describe('with several new rules', () => {
        beforeEach(() => {
          store.modules.approvals.state.rules = [
            { ...rule, id: null, isNew: true },
            { ...rule, id: null, isNew: true },
          ];
        });

        it('renders hidden fields for each rule', () => {
          factory();

          expect(findHiddenInputs().filter((x) => x.name === INPUT_NAME)).toEqual([
            { name: INPUT_NAME, value: rule.name },
            { name: INPUT_NAME, value: rule.name },
          ]);
        });
