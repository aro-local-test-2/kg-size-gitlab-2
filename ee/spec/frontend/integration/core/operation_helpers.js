...
/**
 * Request tracking utilities for frontend integration tests.
 ...
 * @param {Object} [variables] - GraphQL variables, when the request is a GraphQL operation
 */
export function captureRequest(name, request, variables) {
...
/**
 * Returns the GraphQL variables of the most recent call to an operation.
 *
 * Response-only assertions cannot tell two filters apart when they happen to
 * return the same rows; asserting the variables pins down what the app actually
 * requested.
 *
 * @param {string} operationName
 * @returns {Object} variables of the last call
 * @throws {Error} when the operation was never called, or carried no variables
 */
export function lastRequestVariables(operationName) {
  const calls = capturedRequests[operationName];

  if (!calls?.length) {
    const called = Object.keys(capturedRequests).sort().join(', ') || 'none';

    throw new Error(
      `No request captured for "${operationName}", so there are no variables to assert on. Operations called: ${called}.`,
    );
  }

  const { variables } = calls[calls.length - 1];

  if (variables === undefined) {
    throw new Error(
      `"${operationName}" was captured without variables. Only GraphQL operations record them.`,
    );
  }
