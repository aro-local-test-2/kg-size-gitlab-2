...
describe('History Comment', () => {
  ...
  const editButton = () => wrapper.findComponent('[title="Edit Comment"]');
  const deleteButton = () => wrapper.find('[title="Delete Comment"]');
  const confirmDeleteButton = () => wrapper.findComponentByTestId('confirm-delete-button');
  const cancelDeleteButton = () => wrapper.findComponentByTestId('cancel-delete-button');
  ...
  const editAndSaveNewContent = async (content) => {
    await showEditView();
    commentEditor().vm.$emit('save', content);
    return nextTick();
  };
  ...
  describe(`when there's no existing comment`, () => {
    ...
    it('shows the add comment button when the cancel button is clicked in the comment editor', async () => {
      await showEditView();
      await commentEditor().vm.$emit('cancel');
  ...
  `('$desc', ({ propsData, expectedVars, mutationSpyFn, queryName }) => {
    ...
    it('emits event when mutation is successful and resets the state', async () => {
      ...
      wrapper.vm.$on('comment-updated', listener);

      await editAndSaveNewContent('new comment');
      expect(commentEditor().props('isSaving')).toBe(true);
      await waitForPromises();

      expect(wrapper.emitted('comment-updated')).toEqual([[]]);
