...
import AddEditRotationModal from 'ee/oncall_schedules/components/rotations/components/add_edit_rotation_modal.vue';
...
describe('On-call schedule', () => {
  ...
  const findLoadPreviousTimeframeBtn = () =>
    wrapper.findComponentByTestId('previous-timeframe-btn');
  const findLoadNextTimeframeBtn = () => wrapper.findComponentByTestId('next-timeframe-btn');
  ...
  const findAddRotationModal = () => wrapper.findAllComponents(AddEditRotationModal).at(0);
  const findEditRotationModal = () => wrapper.findAllComponents(AddEditRotationModal).at(1);
  ...
  describe('rotation updates', () => {
    it('re-emits "rotation-updated" with the message and options from the add rotation modal', () => {
      findAddRotationModal().vm.$emit('rotation-updated', 'Rotation created', { created: true });

      expect(wrapper.emitted('rotation-updated')).toEqual([
        ['Rotation created', { created: true }],
      ]);
    });

    it('re-emits "rotation-updated" with only the message from the edit rotation modal', () => {
      findEditRotationModal().vm.$emit('rotation-updated', 'Rotation edited');

      expect(wrapper.emitted('rotation-updated')).toEqual([['Rotation edited', undefined]]);
    });
  });
});
