...
module Gitlab
  module Ingestion
    ...
    #   `batch_size`: Optional maximum number of rows per INSERT statement.
    #                 Defaults to `BulkInsertSafe::DEFAULT_BATCH_SIZE`.
    #   `on_conflict`: Optional. `:nothing` skips conflicting rows instead of updating them,
    #                  for tables whose unique key covers every column worth writing.
    #
    # Template methods;
    #
    ...
    module BulkInsertableTask
      ...
      ON_CONFLICT_VALUES = [nil, :nothing].freeze

      def self.included(base)
        base.singleton_class.attr_accessor(:model, :unique_by, :uses, :batch_size)
        base.singleton_class.attr_reader(:on_conflict)
        base.extend(ClassMethods)
      end

      module ClassMethods
        # Checked on assignment so a typo fails when the class loads, instead of
        # quietly taking the DO UPDATE branch this setting exists to avoid.
        def on_conflict=(value)
          unless ON_CONFLICT_VALUES.include?(value)
            raise ArgumentError, "on_conflict must be one of #{ON_CONFLICT_VALUES.inspect}, got #{value.inspect}"
          end

          # rubocop:disable Gitlab/ModuleWithInstanceVariables -- class-level DSL attribute,
          # the same storage the attr_accessor it replaces used
          @on_conflict = value
      ...
      delegate :unique_by, :model, :klass, :uses, :cast_values, :on_conflict, to: :'self.class', private: true

      def insert_batch_size
        self.class.batch_size || BulkInsertSafe::DEFAULT_BATCH_SIZE
      end
        ...
        strong_memoize(:return_data) do
          if insert_objects.present?
            pass_vuln_reads_db_ff do
              unique_by.present? && on_conflict != :nothing ? bulk_upsert : bulk_insert
            end
          else
            []
          end
      ...
      def bulk_insert
        klass.bulk_insert!(insert_objects, skip_duplicates: true, unique_by: unique_by, returns: uses,
          batch_size: insert_batch_size)
      end

      def bulk_upsert
        klass.bulk_upsert!(insert_objects, unique_by: unique_by, returns: uses, batch_size: insert_batch_size) do |attr|
